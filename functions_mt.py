...
"""
def searchlim(array, begin, end, mode):            
    if mode == 'justbegin':
        values, pos = ini(array, begin)
    elif mode == 'justend':
        values, pos = fin(array, end)
    elif mode == 'both':
        values_b, pos_i = ini(array, begin)
        values, pos = fin(values_b, end)
    else:
        print('wrong mode input')
    return values, pos

def ini(array_i, begin_i):
    #Generate the condition
    condition_begin = array_i > begin_i
    values_begin = np.extract(condition_begin, array_i)
    pos_i = len(array_i) - len(values_begin)
    return values_begin, pos_i

def fin(array_e, end_e):
    #Generate the condition
    condition_end = array_e < end_e
    values_end = np.extract(condition_end, array_e)
    pos_e = len(array_e) - len(values_end)
    #Store the cut positionposition_end = len(array_e)-len(values_end)
    return values_end, pos_e
...
def tuneit(data_directory, hd_spikes, wake_ep, mouse_position, neuro_num, nabins, path2save):
    ...
    #firing_rate = nts.Tsd(t = bins[0:-1]+(bin_size/2.), d = spike_count.values/(bin_size/1000./1000.))
    """
    
    """
    Firing rate
    """
    first_spike, last_spike, bin_size, bins, firing_rate, meanfiring = firetdisco(hd_spikes, i, wake_ep)
    """
    Angular direction
    """
    ...
    #plt.bar(phase, tuning_curve, color='darkorange', edgecolor='chocolate')
...
def plotautco(hd_spikes, neuro_num, meanfiring, epoch, epochstr, binsize, nbins, path2save):        
    ...
    array = np.append(np.append((dfa.values),0), np.flipud(dfb.values))
    ...
    width_auto = abs( (((index_max-index_min) * binsize) * 2) + binsize )
    print("for neuron {0} the width is".format(i), width_auto, ' s')
       
    
    """Plot non smoothed autocorrelogram"""
    plt.figure(figsize=(12,8))
    plt.plot(aucorr)
